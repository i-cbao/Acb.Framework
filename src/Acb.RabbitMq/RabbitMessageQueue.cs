...
namespace Acb.RabbitMq
{
    public class RabbitMessageQueue : IMessageQueue
    {
        ...
        public RabbitMessageQueue(IRabbitMqConnection connection, string queue)
        {
            ...
            _logger = LogManager.Logger<RabbitMessageQueue>();
            _queue = queue;
        ...
        public Task<T> Receive<T>()
        {
            ...
            try
            {
                ...
                var model = JsonConvert.DeserializeObject<T>(_encoding.GetString(res.Body));
        ...
        public Task Subscibe<T>(Action<T> action)
        {
            ...
            consumer.Received += (ch, ea) =>
            {
                try
                {
                    var message = JsonConvert.DeserializeObject<T>(_encoding.GetString(ea.Body));
                    action(message);
