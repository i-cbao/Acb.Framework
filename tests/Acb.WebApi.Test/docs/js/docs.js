  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      $.each(jsonData.paths, function(i, path) {
        $.each(path, function(j, o) {
          var tag = o.tags[0]
          for (var index = 0; index < jsonData.tags.length; index++) {
            if (jsonData.tags[index].name == tag) return false
          }
          jsonData.tags.push({ name: tag })
        })
      })
      ...
      $("[name='a_path']").click(function() {
        ...
        $.each(jsonData.paths[path], function(i, d) {
          if (d.operationId == operationId) {
            ...
            if (modelName) {
              $('#path-body-response-model').html(
                tempBodyResponseModel.render(jsonData.definitions[modelName])
              )
            }
          }
        })
